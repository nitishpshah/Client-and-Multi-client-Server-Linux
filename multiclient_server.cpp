...
int main(int argc, char* argv[]){
	...
	for(;;){
		...
		//if accepted connection, let child communicate, 
		//return here and wait for new connections.
		//accept all incoming conections and make a new fork for each
		new_fd = accept(sockfd, (struct sockaddr *)&their_addr, &sin_size);
		int pid = fork();
		if( pid == -1){
			cout<<"error in making a fork"<<endl;
			return -1;
		//skip this part if parent
		//only enter if child
		}else if(pid == 0){

			//accept data in the child process and 
			//shut down the child when the connection is closed
			//the parent doesn't enter this if condition and waits at the start of the for(;;) loop to accept new connections
